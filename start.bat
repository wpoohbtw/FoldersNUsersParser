...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "  $backend = Start-Job -Name FoldersNUsersParserBackend -ScriptBlock { param($root) Set-Location $root; & '.\.venv\Scripts\python.exe' -m uvicorn backend.app.main:app --host 127.0.0.1 --port 8000 --no-access-log 2>&1 } -ArgumentList $root; " ^
  "} " ^
  "if (-not (Test-FnupApi)) { Write-Host '[BACKEND] Waiting for backend readiness...'; $ready = $false; for ($i = 0; $i -lt 80; $i++) { if ($backend) { Receive-Job $backend | ForEach-Object { Write-Host ('[BACKEND] ' + $_) }; if ($backend.State -ne 'Running') { Write-Host '[BACKEND] ERROR: backend stopped before readiness.'; exit 1 } }; if (Test-FnupApi) { $ready = $true; break }; Start-Sleep -Milliseconds 250 }; if (-not $ready) { Write-Host '[BACKEND] ERROR: backend did not become ready in time.'; exit 1 } } " ^
